...
Product::Product (const ValueTree& s) :
    state (s)
{
...
namespace ProductFinder
{
    ...
    #define RETURN_TYPE_WITH_NAME(enumClassName, varName) \
        case enumClassName::varName: return JUCE_STRINGIFY (varName);
    ...
    String toString (Status s)
    {
        switch (s)
        {
            ...
            case Status::privatised: return "private";
            default: break;
        };
    ...
    String toString (TaxClass tc)
    {
        switch (tc)
        {
            case TaxClass::reducedRate: return "reduced-rate";
            case TaxClass::zeroRate:    return "zero-rate";
            default: break;
        };
    ...
    String toString (StockStatus ss)
    {
        switch (ss)
        {
            case StockStatus::outOfStock:   return "outofstock";
            case StockStatus::onBackOrder:  return "onbackorder";
            default: break;
        };
    ...
    Context createContextFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("edit"))        return Context::edit;
        else if (r.equalsIgnoreCase ("view"))   return Context::view;
    ...
    Status createStatusFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("draft"))               return Status::draft;
        else if (r.equalsIgnoreCase ("pending"))        return Status::pending;
        else if (r.equalsIgnoreCase ("privatised"))     return Status::privatised;
        else if (r.equalsIgnoreCase ("publish"))        return Status::publish;
        else if (r.equalsIgnoreCase ("any"))            return Status::any;
    ...
    Type createTypeFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("grouped"))         return Type::grouped;
        else if (r.equalsIgnoreCase ("external"))   return Type::external;
        else if (r.equalsIgnoreCase ("variable"))   return Type::variable;
        else if (r.equalsIgnoreCase ("simple"))     return Type::simple;
    ...
    SortOrder createSortOrderFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("id"))              return SortOrder::id;
        else if (r.equalsIgnoreCase ("include"))    return SortOrder::include;
        else if (r.equalsIgnoreCase ("title"))      return SortOrder::title;
        else if (r.equalsIgnoreCase ("slug"))       return SortOrder::slug;
        else if (r.equalsIgnoreCase ("date"))       return SortOrder::date;
    ...
    TaxClass createTaxClassFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("reduced-rate"))    return TaxClass::reducedRate;
        else if (r.equalsIgnoreCase ("zero-rate"))  return TaxClass::zeroRate;
        else if (r.equalsIgnoreCase ("standard"))   return TaxClass::standard;
    ...
    StockStatus createStockStatusFromString (const String& s)
    {
        ...
        if (r.equalsIgnoreCase ("outofstock"))      return StockStatus::outOfStock;
        else if (r.equalsIgnoreCase ("backorder"))  return StockStatus::onBackOrder;
        else if (r.equalsIgnoreCase ("instock"))    return StockStatus::inStock;
    ...
    void SearchParameters::populate (StringPairArray& spa) const
    {
        ...
        if (before != nullptr)  addIfNotEmpty (spa, "before", before->toISO8601 (true));
        if (after != nullptr)   addIfNotEmpty (spa, "after", after->toISO8601 (true));
        ...
        if (isAscending)    addIfNotEmpty (spa, "order", "asc"); //NB: default when unspecified is descending ("desc")
        if (mustBeFeatured) addIfNotEmpty (spa, "featured", "1");
        if (mustBeOnSale)   addIfNotEmpty (spa, "on_sale", "1");

        if (pageIndex != 1) spa.set ("page", String (pageIndex));
        if (pageSize != 10) spa.set ("per_page", String (std::clamp (pageSize, 1, 1000)));
        if (offset > 0)     spa.set ("offset", String (std::clamp (offset, 1, 1000)));
