...
static uint32 Hash32Len13to24(const char* s, size_t len)
{
    ...
    auto b = Fetch32(s + 4);
    auto c = Fetch32(s + len - 8);
    auto d = Fetch32(s + (len >> 1));
    auto e = Fetch32(s);
    auto f = Fetch32(s + len - 4);
...
static uint32 Hash32Len0to4(const char* s, size_t len)
{
    ...
    {
        auto v = static_cast<signed char>(s[i]);
...
uint32 CityHash::hash32(const char* s, size_t len)
{
    ...
    auto a0 = Rotate32(Fetch32(s + len - 4) * c1, 17) * c2;
    auto a1 = Rotate32(Fetch32(s + len - 8) * c1, 17) * c2;
    auto a2 = Rotate32(Fetch32(s + len - 16) * c1, 17) * c2;
    auto a3 = Rotate32(Fetch32(s + len - 12) * c1, 17) * c2;
    auto a4 = Rotate32(Fetch32(s + len - 20) * c1, 17) * c2;
    h ^= a0;
    h = Rotate32(h, 19);
    h = h * 5 + 0xe6546b64;
    h ^= a2;
    ...
    do
    {
        auto a0 = Rotate32(Fetch32(s) * c1, 17) * c2;
        auto a1 = Fetch32(s + 4);
        auto a2 = Rotate32(Fetch32(s + 8) * c1, 17) * c2;
        auto a3 = Rotate32(Fetch32(s + 12) * c1, 17) * c2;
        auto a4 = Fetch32(s + 16);
        h ^= a0;
        h = Rotate32(h, 18);
        h = h * 5 + 0xe6546b64;
        f += a1;
...
uint64 CityHash::hash64(const char* s, size_t len, uint64 seed)
{
    return hash64(s, len, k2, seed);
}

uint64 CityHash::hash64(const char* s, size_t len, uint64 seed0, uint64 seed1)
{
