namespace
{
    constexpr uint8 sbox[16][16] =
    {
        { 0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76 },
        { 0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0 },
        { 0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15 },
        { 0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75 },
        { 0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84 },
        { 0x53, 0xd1, 0x00, 0xed, 0x20, 0xfc, 0xb1, 0x5b, 0x6a, 0xcb, 0xbe, 0x39, 0x4a, 0x4c, 0x58, 0xcf },
        { 0xd0, 0xef, 0xaa, 0xfb, 0x43, 0x4d, 0x33, 0x85, 0x45, 0xf9, 0x02, 0x7f, 0x50, 0x3c, 0x9f, 0xa8 },
        { 0x51, 0xa3, 0x40, 0x8f, 0x92, 0x9d, 0x38, 0xf5, 0xbc, 0xb6, 0xda, 0x21, 0x10, 0xff, 0xf3, 0xd2 },
        { 0xcd, 0x0c, 0x13, 0xec, 0x5f, 0x97, 0x44, 0x17, 0xc4, 0xa7, 0x7e, 0x3d, 0x64, 0x5d, 0x19, 0x73 },
        { 0x60, 0x81, 0x4f, 0xdc, 0x22, 0x2a, 0x90, 0x88, 0x46, 0xee, 0xb8, 0x14, 0xde, 0x5e, 0x0b, 0xdb },
        { 0xe0, 0x32, 0x3a, 0x0a, 0x49, 0x06, 0x24, 0x5c, 0xc2, 0xd3, 0xac, 0x62, 0x91, 0x95, 0xe4, 0x79 },
        { 0xe7, 0xc8, 0x37, 0x6d, 0x8d, 0xd5, 0x4e, 0xa9, 0x6c, 0x56, 0xf4, 0xea, 0x65, 0x7a, 0xae, 0x08 },
        { 0xba, 0x78, 0x25, 0x2e, 0x1c, 0xa6, 0xb4, 0xc6, 0xe8, 0xdd, 0x74, 0x1f, 0x4b, 0xbd, 0x8b, 0x8a },
        { 0x70, 0x3e, 0xb5, 0x66, 0x48, 0x03, 0xf6, 0x0e, 0x61, 0x35, 0x57, 0xb9, 0x86, 0xc1, 0x1d, 0x9e },
        { 0xe1, 0xf8, 0x98, 0x11, 0x69, 0xd9, 0x8e, 0x94, 0x9b, 0x1e, 0x87, 0xe9, 0xce, 0x55, 0x28, 0xdf },
        { 0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16 }
    };

    constexpr uint8 inv_sbox[16][16] =
    {
        { 0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb },
        { 0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb },
        { 0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42, 0xfa, 0xc3, 0x4e },
        { 0x08, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25 },
        { 0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, 0xa4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92 },
        { 0x6c, 0x70, 0x48, 0x50, 0xfd, 0xed, 0xb9, 0xda, 0x5e, 0x15, 0x46, 0x57, 0xa7, 0x8d, 0x9d, 0x84 },
        { 0x90, 0xd8, 0xab, 0x00, 0x8c, 0xbc, 0xd3, 0x0a, 0xf7, 0xe4, 0x58, 0x05, 0xb8, 0xb3, 0x45, 0x06 },
        { 0xd0, 0x2c, 0x1e, 0x8f, 0xca, 0x3f, 0x0f, 0x02, 0xc1, 0xaf, 0xbd, 0x03, 0x01, 0x13, 0x8a, 0x6b },
        { 0x3a, 0x91, 0x11, 0x41, 0x4f, 0x67, 0xdc, 0xea, 0x97, 0xf2, 0xcf, 0xce, 0xf0, 0xb4, 0xe6, 0x73 },
        { 0x96, 0xac, 0x74, 0x22, 0xe7, 0xad, 0x35, 0x85, 0xe2, 0xf9, 0x37, 0xe8, 0x1c, 0x75, 0xdf, 0x6e },
        { 0x47, 0xf1, 0x1a, 0x71, 0x1d, 0x29, 0xc5, 0x89, 0x6f, 0xb7, 0x62, 0x0e, 0xaa, 0x18, 0xbe, 0x1b },
        { 0xfc, 0x56, 0x3e, 0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4 },
        { 0x1f, 0xdd, 0xa8, 0x33, 0x88, 0x07, 0xc7, 0x31, 0xb1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xec, 0x5f },
        { 0x60, 0x51, 0x7f, 0xa9, 0x19, 0xb5, 0x4a, 0x0d, 0x2d, 0xe5, 0x7a, 0x9f, 0x93, 0xc9, 0x9c, 0xef },
        { 0xa0, 0xe0, 0x3b, 0x4d, 0xae, 0x2a, 0xf5, 0xb0, 0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61 },
        { 0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d }
    };

    /** Galois Multiplication lookup tables

        @see https://en.wikipedia.org/wiki/Rijndael_MixColumns#Galois_Multiplication_lookup_tables
    */
    constexpr uint8 GF_MUL_TABLE[15][256] =
    {
        {}, {},

        // mul 2
        {
            
        },

        // mul 3
        {
            
        },

        {}, {},  {}, {}, {},

        // mul 9
        {
            
        },

        {},

        // mul 11
        {
            
        },

        {},

        // mul 13
        {
            
        },

        // mul 14
        {
            
        }
    };

    //==============================================================================
    // circulant MDS matrix
    static const uint8 CMDS[4][4]       = { { 2, 3, 1, 1 }, { 1, 2, 3, 1 }, { 1, 1, 2, 3 }, { 3, 1, 1, 2 } };

    // Inverse circulant MDS matrix
    static const uint8 INV_CMDS[4][4]   = { { 14, 11, 13, 9 }, { 9, 14, 11, 13 }, { 13, 9, 14, 11 }, { 11, 13, 9, 14 } };

    /** Plaintext length must be divisible by blockBytesLen. */
    constexpr bool checkLength (size_t len, size_t blockBytesLen)
    {
        return (len % blockBytesLen) == 0;
    }
}

//==============================================================================
AES::AES (const void* keyData, size_t keyBytes, Mode m) :
    key (keyData, keyBytes),
    mode (m)
{
    jassert (keyBytes == 128 || keyBytes == 192 || keyBytes == 256);

    switch (keyBytes)
    {
        case 128:
        {
            Nk = 4;
            Nr = 10;
        }
        break;

        case 192:
        {
            Nk = 6;
            Nr = 12;
        }
        break;

        case 256:
        {
            Nk = 8;
            Nr = 14;
        }
        break;

        default:
            jassertfalse; // You need to pick a standard size!
        break;
    }

    expandKey();
}

//==============================================================================
std::array<uint8, 256> AES::createSBOX()
{
    std::array<uint8, 256> r;
    uint8 p = 1, q = 1;

	// loop invariant: p * q == 1 in the Galois field 
	do
    {
		// multiply p by 3
		p = p ^ (p << 1) ^ (p & 0x80 ? 0x1B : 0);

		// divide q by 3 (equals multiplication by 0xf6)
		q ^= q << 1;
		q ^= q << 2;
		q ^= q << 4;
		q ^= q & 0x80 ? 0x09 : 0;

        #undef ROTL8
        #define ROTL8(x,shift) ((uint8_t) ((x) << (shift)) | ((x) >> (8 - (shift))))

		// compute the affine transformation */
		const uint8 xformed = q ^ ROTL8 (q, 1) ^ ROTL8 (q, 2) ^ ROTL8 (q, 3) ^ ROTL8 (q, 4);
        #undef ROTL8

		r[p] = xformed ^ 0x63;
	}
    while (p != 1);

	r[0] = 0x63; // 0 is a special case since it has no inverse

    return r;
}

//==============================================================================
MemoryBlock AES::encryptECB (const MemoryBlock& in)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);

    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
        encryptBlock ((uint8*) (in.getData()) + i, (uint8*) (out.getData()) + i);

    return out;
}

MemoryBlock AES::encryptCBC (const MemoryBlock& in, const MemoryBlock& iv)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);

    uint8 block[blockBytesLen];
    memcpy (block, iv.getData(), blockBytesLen);
    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
    {
        xorBlocks (block, (uint8*) (in.getData()) + i, block, blockBytesLen);
        encryptBlock (block, (uint8*) (out.getData()) + i);
        memcpy (block, (uint8*) (out.getData()) + i, blockBytesLen);
    }

    return out;
}

MemoryBlock AES::encryptCFB (const MemoryBlock& in, const MemoryBlock& iv)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);

    uint8 block[blockBytesLen];
    uint8 encryptedBlock[blockBytesLen];
    memcpy (block, iv.getData(), blockBytesLen);
    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
    {
        encryptBlock (block, encryptedBlock);
        xorBlocks ((uint8*) (in.getData()) + i,
                   encryptedBlock,
                   (uint8*) (out.getData()) + i,
                   blockBytesLen);
        memcpy (block, (uint8*) (out.getData()) + i, blockBytesLen);
    }

    return out;
}

//==============================================================================
MemoryBlock AES::decryptECB (const MemoryBlock& in)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);

    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
        decryptBlock ((uint8*) (in.getData()) + i, (uint8*) (out.getData()) + i);

    return out;
}

MemoryBlock AES::decryptCBC (const MemoryBlock& in, const MemoryBlock& iv)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);
    uint8 block[blockBytesLen];
    memcpy (block, iv.getData(), blockBytesLen);
    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
    {
        decryptBlock ((uint8*) (in.getData()) + i, (uint8*) (out.getData()) + i);
        xorBlocks (block,
                   (uint8*) (out.getData()) + i,
                   (uint8*) (out.getData()) + i, 
                   blockBytesLen);
        memcpy (block, (uint8*) (in.getData()) + i, blockBytesLen);
    }

    return out;
}

MemoryBlock AES::decryptCFB (const MemoryBlock& in, const MemoryBlock& iv)
{
    if (! checkLength (in.getSize(), blockBytesLen))
        return {};

    MemoryBlock out (in.getSize(), true);

    uint8 block[blockBytesLen];
    uint8 encryptedBlock[blockBytesLen];
    memcpy (block, iv.getData(), blockBytesLen);
    for (size_t i = 0; i < out.getSize(); i += blockBytesLen)
    {
        encryptBlock (block, encryptedBlock);
        xorBlocks ((uint8*) (in.getData()) + i,
                   encryptedBlock,
                   (uint8*) (out.getData()) + i,
                   blockBytesLen);
        memcpy (block, (uint8*) (in.getData()) + i, blockBytesLen);
    }

    return out;
}

//==============================================================================
void AES::encryptBlock (const uint8* in, uint8* out)
{
    State state;

    for (size_t i = 0; i < 4; ++i)
        for (size_t j = 0; j < Nb; ++j)
            state[i][j] = in[i + 4 * j];

    addRoundKey (state, roundKeys.data());

    for (size_t round = 1; round <= Nr - 1; round++)
    {
        subBytes (state);
        shiftRows (state);
        mixColumns (state);
        addRoundKey (state, roundKeys.data() + round * 4 * Nb);
    }

    subBytes (state);
    shiftRows (state);
    addRoundKey (state, roundKeys.data() + Nr * 4 * Nb);

    for (size_t i = 0; i < 4; ++i)
        for (size_t j = 0; j < Nb; ++j)
            out[i + 4 * j] = state[i][j];
}

void AES::decryptBlock (const uint8* in, uint8* out)
{
    State state;

    for (size_t i = 0; i < 4; ++i)
        for (size_t j = 0; j < Nb; ++j)
            state[i][j] = in[i + 4 * j];

    addRoundKey (state, roundKeys.data() + Nr * 4 * Nb);

    for (size_t round = Nr - 1; round >= 1; round--)
    {
        invSubBytes (state);
        invShiftRows (state);
        addRoundKey (state, roundKeys.data() + round * 4 * Nb);
        invMixColumns (state);
    }

    invSubBytes (state);
    invShiftRows (state);
    addRoundKey (state, roundKeys.data());

    for (size_t i = 0; i < 4; ++i)
        for (size_t j = 0; j < Nb; ++j)
            out[i + 4 * j] = state[i][j];
}

//==============================================================================
void AES::subBytes (State& state)
{
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < Nb; ++j)
        {
            const auto t = state[i][j];
            state[i][j] = sbox[t / 16][t % 16];
        }
    }
}

void AES::shiftRow (State& state, size_t i, size_t n)
{
    InnerState inner;
    for (size_t j = 0; j < Nb; ++j)
        inner[j] = state[i][(j + n) % Nb];

    state[i] = inner;
}

void AES::shiftRows (State& state)
{
    shiftRow (state, 1, 1);
    shiftRow (state, 2, 2);
    shiftRow (state, 3, 3);
}

void AES::mixColumns (State& state)
{
    State temp;

    for (size_t i = 0; i < 4; ++i)
        temp[i].fill (0);

    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t k = 0; k < 4; ++k)
        {
            for (size_t j = 0; j < 4; ++j)
            {
                if (CMDS[i][k] == 1)
                    temp[i][j] ^= state[k][j];
                else
                    temp[i][j] ^= GF_MUL_TABLE[CMDS[i][k]][state[k][j]];
            }
        }
    }

    for (size_t i = 0; i < 4; ++i)
        state[i] = temp[i];
}

void AES::addRoundKey (State& state, uint8* k)
{
    for (size_t i = 0; i < 4; ++i)
        for (size_t j = 0; j < Nb; ++j)
            state[i][j] ^= k[i + Nb * j];
}

void AES::subWord (uint8* a)
{
    for (int i = 0; i < 4; ++i)
        a[i] = sbox[a[i] / 16][a[i] % 16];
}

void AES::rotWord (uint8* a)
{
    auto c = a[0];
    a[0] = a[1];
    a[1] = a[2];
    a[2] = a[3];
    a[3] = c;
}

void AES::xorWords (uint8* a, uint8* b, uint8* c)
{
    for (int i = 0; i < 4; ++i)
        c[i] = a[i] ^ b[i];
}

void AES::rcon (uint8* a, size_t n)
{
    // multiply on x
    uint8 c = 1;
    for (size_t i = 0; i < n - 1; ++i)
        c = (c << 1) ^ (((c >> 7) & 1) * 0x1b);

    a[0] = c;
    a[1] = a[2] = a[3] = 0;
}

void AES::expandKey()
{
    uint8 temp[4];
    uint8 rconVar[4];

    size_t i = 0;
    while (i < 4 * Nk)
    {
        roundKeys[i] = ((uint8*) key.getData())[i];
        ++i;
    }

    i = 4 * Nk;
    while (i < 4 * Nb * (Nr + 1))
    {
        temp[0] = roundKeys[i - 4 + 0];
        temp[1] = roundKeys[i - 4 + 1];
        temp[2] = roundKeys[i - 4 + 2];
        temp[3] = roundKeys[i - 4 + 3];

        if (i / 4 % Nk == 0)
        {
            rotWord (temp);
            subWord (temp);
            rcon (rconVar, i / (Nk * 4));
            xorWords (temp, rconVar, temp);
        }
        else if (Nk > 6 && i / 4 % Nk == 4)
        {
            subWord (temp);
        }

        roundKeys[i + 0] = roundKeys[i - 4 * Nk]     ^ temp[0];
        roundKeys[i + 1] = roundKeys[i + 1 - 4 * Nk] ^ temp[1];
        roundKeys[i + 2] = roundKeys[i + 2 - 4 * Nk] ^ temp[2];
        roundKeys[i + 3] = roundKeys[i + 3 - 4 * Nk] ^ temp[3];
        i += 4;
    }
}

void AES::invSubBytes (State& state)
{
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < Nb; ++j)
        {
            const auto t = state[i][j];
            state[i][j] = inv_sbox[t / 16][t % 16];
        }
    }
}

void AES::invMixColumns (State& state)
{
    State temp;

    for (auto& ts : temp)
        ts.fill (0);

    for (size_t i = 0; i < 4; ++i)
        for (size_t k = 0; k < 4; ++k)
            for (size_t j = 0; j < 4; ++j)
                temp[i][j] ^= GF_MUL_TABLE[INV_CMDS[i][k]][state[k][j]];

    for (size_t i = 0; i < 4; ++i)
        state[i] = temp[i];
}

void AES::invShiftRows (State& state)
{
    shiftRow (state, 1, Nb - 1);
    shiftRow (state, 2, Nb - 2);
    shiftRow (state, 3, Nb - 3);
}

void AES::xorBlocks (const uint8* a, const uint8* b, uint8* c, size_t len)
{
    for (size_t i = 0; i < len; ++i)
        c[i] = a[i] ^ b[i];
}
