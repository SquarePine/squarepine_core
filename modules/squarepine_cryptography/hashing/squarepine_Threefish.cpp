Threefish::Threefish (const void* const keyData, const int keyBytes)
{
...
Threefish::Threefish (const Threefish& other)
{
    for (auto& block : s)
        block.malloc (256);
...
Threefish& Threefish::operator= (const Threefish& other) noexcept
{
...
Threefish::~Threefish() noexcept {}

uint32 Threefish::F (const uint32 x) const noexcept
{
    return ((s[0][(x >> 24) & 0xff] + s[1][(x >> 16) & 0xff])
                ^ s[2][(x >> 8) & 0xff]) + s[3][x & 0xff];
}

void Threefish::encrypt (uint32& data1, uint32& data2) const noexcept
{
    auto l = data1;
    auto r = data2;
...
void Threefish::decrypt (uint32& data1, uint32& data2) const noexcept
{
    auto l = data1;
    auto r = data2;
...
void Threefish::encrypt (MemoryBlock& data) const
{
...
void Threefish::decrypt (MemoryBlock& data) const
{
...
int Threefish::encrypt (void* data, size_t size, size_t bufferSize) const noexcept
{
    auto encryptedSize = pad (data, size, bufferSize);

    if (encryptedSize >= 0 && apply (data, static_cast<size_t> (encryptedSize), &Threefish::encrypt))
        return encryptedSize;

    return -1;
}

int Threefish::decrypt (void* data, size_t size) const noexcept
{
    if (apply (data, size, &Threefish::decrypt))
        return unpad (data, size);

    return -1;
}

bool Threefish::apply (void* data, size_t size, void (Threefish::*op) (uint32&, uint32&) const) const
{
...
int Threefish::pad (void* data, size_t size, size_t bufferSize) noexcept
{
...
int Threefish::unpad (const void* data, size_t size) noexcept
{
    if (size == 0)
        return -1;
...
class ThreeFishTests final : public UnitTest
{
public:
    ThreeFishTests()
        : UnitTest ("Threefish", UnitTestCategories::cryptography)
    {}
    ...
    void encryptDecryptTest (const Threefish& Threefish, void* data, size_t size, size_t bufferSize)
    {
        MemoryBlock copy (data, size);

        int encryptedSize = Threefish.encrypt (data, size, bufferSize);
        expectGreaterThan (encryptedSize, (int) size);
        expectLessOrEqual (encryptedSize, (int) bufferSize);

        int decryptedSize = Threefish.decrypt (data, static_cast<size_t> (encryptedSize));
        expectEquals ((int) size, decryptedSize);

        expectEqualData (data, copy.getData(), size, "Length/Content changed during encryption");
    }

    void encryptDecryptTest (const Threefish& Threefish, MemoryBlock& data)
    {
        MemoryBlock copy (data);

        Threefish.encrypt (data);
        Threefish.decrypt (data);

        expectEqualMemoryBlocks (data, copy, "Length/Content changed during encryption");
    }

    void encryptDecryptTest (const Threefish& Threefish, const String& data)
    {
        MemoryBlock block (data.toRawUTF8(), static_cast<size_t> (data.length()));
        encryptDecryptTest (Threefish, block);
    }
    ...
    void runTest() override
    {
        ...
        for (int i = 0; i < 100; ++i)
        {
            ...
            Threefish bf (key.getData(), keySize);
...
static ThreeFishTests ThreeFishUnitTests;
