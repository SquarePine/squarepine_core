Twofish::Twofish (const void* const keyData, const int keyBytes)
{
...
Twofish::Twofish (const Twofish& other)
{
    for (auto& block : s)
        block.malloc (256);
...
Twofish& Twofish::operator= (const Twofish& other) noexcept
{
...
uint32 Twofish::F (const uint32 x) const noexcept
{
    return ((s[0][(x >> 24) & 0xff] + s[1][(x >> 16) & 0xff])
                ^ s[2][(x >> 8) & 0xff]) + s[3][x & 0xff];
}

void Twofish::encrypt (uint32& data1, uint32& data2) const noexcept
{
    auto l = data1;
    auto r = data2;
...
void Twofish::decrypt (uint32& data1, uint32& data2) const noexcept
{
    auto l = data1;
    auto r = data2;
...
void Twofish::encrypt (MemoryBlock& data) const
{
...
void Twofish::decrypt (MemoryBlock& data) const
{
...
int Twofish::encrypt (void* data, size_t size, size_t bufferSize) const noexcept
{
    auto encryptedSize = pad (data, size, bufferSize);

    if (encryptedSize >= 0 && apply (data, static_cast<size_t> (encryptedSize), &Twofish::encrypt))
        return encryptedSize;

    return -1;
}

int Twofish::decrypt (void* data, size_t size) const noexcept
{
    if (apply (data, size, &Twofish::decrypt))
        return unpad (data, size);

    return -1;
}

bool Twofish::apply (void* data, size_t size, void (Twofish::*op) (uint32&, uint32&) const) const
{
...
int Twofish::pad (void* data, size_t size, size_t bufferSize) noexcept
{
...
int Twofish::unpad (const void* data, size_t size) noexcept
{
    if (size == 0)
        return -1;
...
class TwoFishTests final : public UnitTest
{
public:
    TwoFishTests()
        : UnitTest ("Twofish", UnitTestCategories::cryptography)
    {}
    ...
    void encryptDecryptTest (const Twofish& Twofish, void* data, size_t size, size_t bufferSize)
    {
        MemoryBlock copy (data, size);

        int encryptedSize = Twofish.encrypt (data, size, bufferSize);
        expectGreaterThan (encryptedSize, (int) size);
        expectLessOrEqual (encryptedSize, (int) bufferSize);

        int decryptedSize = Twofish.decrypt (data, static_cast<size_t> (encryptedSize));
        expectEquals ((int) size, decryptedSize);

        expectEqualData (data, copy.getData(), size, "Length/Content changed during encryption");
    }

    void encryptDecryptTest (const Twofish& Twofish, MemoryBlock& data)
    {
        MemoryBlock copy (data);

        Twofish.encrypt (data);
        Twofish.decrypt (data);

        expectEqualMemoryBlocks (data, copy, "Length/Content changed during encryption");
    }

    void encryptDecryptTest (const Twofish& Twofish, const String& data)
    {
        MemoryBlock block (data.toRawUTF8(), static_cast<size_t> (data.length()));
        encryptDecryptTest (Twofish, block);
    }
    ...
    void runTest() override
    {
        ...
        for (int i = 0; i < 100; ++i)
        {
            ...
            Twofish bf (key.getData(), keySize);
...
static TwoFishTests TwoFishUnitTests;
